...
def main():
	...
	while(len(processos) > 0):
		for it in range(len(processos)):
			...
			for i in range(3):
				atuais = processos[it]._atuais_recursos[i] 				#recursos atuais do processo
				max_processo_recursos = processos[it]._max_recursos[i] 	#maximo derecursos que o processo precisa
				disponivel = recursos_disponiveis[i]					#recursos disponiveis
				if(max_processo_recursos - atuais <= disponivel):
					indice = i
					recursos_disponiveis[i] = disponivel + atuais
					processos.pop(it)
					achou = True
			estado.append(achou)
